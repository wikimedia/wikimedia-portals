...
casper.test.begin( 'Wikipedia Portal - abtest3', function suite( test ) {

  var portalUrl = casper.cli.get( 'url' );

  var abTestGroup = 'language-detection-b';
  ...
  var elTests = {
    clickEvent: function( section ) {
      ...
      if ( section === elSections['primary links'] ) {
      	var anchor = casper.getElementInfo(section.sectionSelector + ' a');
      	test.assertEquals( anchor.attributes.href, '//'+browserLang+'.wikipedia.org/',
      	'First top-link correctly arranged.' );
      }
    },
